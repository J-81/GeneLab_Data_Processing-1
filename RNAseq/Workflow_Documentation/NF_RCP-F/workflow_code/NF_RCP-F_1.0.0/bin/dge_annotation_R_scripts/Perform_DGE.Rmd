...
#group <- make.names(group) # group naming compatible with R models, DEFER make.names UNTIL CONTRASTS to prevent misordering due to make.names adding 'X' to cases with leading integers like '1G'
...
contrasts <- combn(levels(factor(make.names(group))),2) # generate matrix of pairwise group combinations for comparison
...
sampleTable <- data.frame(condition = factor(make.names(group_sub))) # create a new sampleTable with only the kept samples
...
write.csv(
    sampleTable,
    file =  paste0(params$normalized_counts_output_prefix, "SampleTable.csv")
)
